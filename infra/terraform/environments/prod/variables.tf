...
variable "vpc_cidr" {
  description = "CIDR for the production VPC. Non-overlapping with dev and staging."
  type        = string
...
variable "data_volume_size_gb" {
  description = "Size of the volume holding the SQLite database. The disk alarm fires at 75% used, so leave headroom for growth between capacity reviews."
  type        = number
  default     = 100
}
...
variable "allowed_ingress_cidrs" {
  ...
  validation {
    ...
    error_message = "allowed_ingress_cidrs must be set explicitly in production. Use [\"0.0.0.0/0\"] if the API is genuinely public; the point is that it is a stated decision rather than an inherited default."
  }
}
...
variable "certificate_arn" {
  ...
  validation {
    ...
    error_message = "certificate_arn must be a valid ACM certificate ARN. Production must not serve the API over plain HTTP."
  }
}
...
variable "signature_verification_enabled" {
  ...
  validation {
    ...
    error_message = "signature_verification_enabled must be true in production. backend/.env.example states it is mandatory on mainnet deployments; disabling it would accept unsigned requests on a service that moves funds."
  }
}
...
variable "alert_email_addresses" {
  ...
  validation {
    ...
    error_message = "At least one alert email address is required in production."
  }
}
